...
st.set_page_config(page_title="Vistoria Técnica Pro v8", page_icon="🏠", layout="centered")
    ...
    with st.expander(f"📍 {titulo_secao}", expanded=False):
        ...
        if st.checkbox("Incluir Piso/Rodapé?", key=f"inc_piso_{id_chave}"):
            ...
            if not any(x in nome_lc for x in ["cozinha", "banheiro", "serviço"]) or st.checkbox("Tem rodapé?", key=f"tem_roda_{id_chave}"):
                ...
                texto_acumulado += f"- Rodapé {r_mat} na cor {r_cor}, {r_est}.\n"
        ...
        if st.checkbox("Incluir Paredes/Teto?", key=f"inc_par_{id_chave}"):
            ...
            if tipo_p == "Alvenaria":
                c1, c2 = st.columns(2)
                pc = c1.selectbox("Cor Parede", OPCOES_CORES, key=f"pc_{id_chave}")
                pe = c2.selectbox("Estado Parede", OPCOES_ESTADO, key=f"pe_{id_chave}")
        ...
        if any(x in nome_lc for x in ["cozinha", "banheiro"]):
            ...
            if st.checkbox("Incluir Pia?", key=f"inc_pia_{id_chave}"):
                c1, c2, c3 = st.columns(3)
                pm = c1.selectbox("Pia", OPCOES_MAT_PIA, key=f'pm_{id_chave}')
                cm = c2.selectbox("Cuba", OPCOES_MAT_CUBA, key=f'cm_{id_chave}')
                pse = c3.selectbox("Est. Pia", OPCOES_ESTADO, key=f'pse_{id_chave}')
        ...
        # --- 4. PORTA (TUDO NA MESMA LINHA + CHAVES) ---
        if st.checkbox("Incluir Porta?", key=f"inc_por_{id_chave}"):
            st.markdown("---")
            st.markdown("#### 4. Porta")
            c1, c2 = st.columns(2)
            p_mat = c1.selectbox("Material Porta", ["madeira", "alumínio", "ferro"], key=f"pm_p_{id_chave}")
            ...
            tem_chave = ch_col1.checkbox("Tem chave?", key=f"tem_ch_{id_chave}")
            qtd_ch = ch_col2.number_input("Qtd chaves", 0, 10, key=f"qtd_ch_{id_chave}") if tem_chave else 0
            
            txt_chave = f", acompanhada de {qtd_ch} {plural(qtd_ch, 'chave', 'chaves')}" if tem_chave else ", sem chave"
            
            texto_acumulado += f"- Porta de {p_mat} {p_est}, batente {bat_e}, maçaneta {mac_e} e fechadura {fec_e}{txt_chave}.\n"
        ...
        if st.checkbox("Incluir Iluminação?", key=f"inc_ilu_{id_chave}"):
            ...
            c1, c2 = st.columns(2)
            q_ilu = c1.number_input("Quantidade", 1, 20, key=f"qilu_{id_chave}")
            e_ilu = c2.selectbox("Estado", OPCOES_ESTADO, key=f"eilu_{id_chave}")
            
            detalhe = ""
            if t_ilu == "plafon": detalhe = "de vidro "
            elif t_ilu == "spot": detalhe = f"de {st.selectbox('Material Spot', OPCOES_MAT_SPOT, key=f'mspot_{id_chave}')} "
            
            texto_acumulado += f"- Iluminação com {q_ilu:02} {plural(q_ilu, t_ilu, t_ilu+'s')} {detalhe}{e_ilu}.\n"

        # --- 7. ELÉTRICA ---
        if st.checkbox("Incluir Elétrica?", key=f"inc_ele_{id_chave}"):
            c1, c2, c3 = st.columns(3)
            tom, inter = c1.number_input('Tomadas', 0, 50, key=f't_{id_chave}'), c2.number_input('Interr.', 0, 50, key=f'i_{id_chave}')
            ee = c3.selectbox('Est. Placas', OPCOES_ESTADO, key=f'ee_{id_chave}')
            texto_acumulado += f"- Elétrica com {tom} tomadas e {inter} interruptores, {ee}.\n"
            if st.checkbox("Incluir Quadro de Disjuntores?", key=f"inc_quad_{id_chave}"):
                qc1, qc2 = st.columns(2)
                q_mat, q_est = qc1.selectbox("Material Quadro", ["plástico", "metal"], key=f"qm_{id_chave}"), qc2.selectbox("Estado Quadro", OPCOES_ESTADO, key=f"qe_{id_chave}")
                texto_acumulado += f"- Quadro de disjuntores em {q_mat} {q_est}.\n"
        ...
        if st.checkbox("Incluir Ralo?", key=f"inc_ral_{id_chave}"):
            ...
            texto_acumulado += f"- Ralo com {r_qtd:02} {plural(r_qtd, 'unidade', 'unidades')} de {r_mat} em bom estado.\n"
...
if relatorio:
    ...
    html_format = f"""<html><head><style>body {{ font-family: 'Times New Roman'; font-size: 12pt; color: black; line-height: 1.2; }}</style></head>
    <body>{relatorio.replace(chr(10), '<br>')}</body></html>"""
