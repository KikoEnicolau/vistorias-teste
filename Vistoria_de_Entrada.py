...
st.set_page_config(page_title="Vistoria Técnica Pro v7", page_icon="🏠", layout="centered")
...
# --- APP ---
st.title("📋 Gerador de Vistorias Profissional")
q_q, q_s = st.columns(2)
num_q = q_q.number_input("Quartos", 0, 10, 1)
num_s = q_s.number_input("Suítes", 0, 10, 0)
areas = st.multiselect("Áreas do Imóvel:", ["Sala", "Corredor", "Cozinha", "Banheiro Social", "Área de Serviço"], default=["Sala", "Cozinha"])

relatorio = ""

# Lógica dos cômodos selecionados
if "Sala" in areas:
...
if "Corredor" in areas:
    relatorio += formulario_base("corredor", "Corredor")
...
if relatorio:
    ...
    st.text_area("Pré-visualização do Texto:", relatorio, height=250)
    
    # Gerador de HTML para o Word (Times New Roman 12)
    html_format = f"""
    <html>
    <head><style>
        body {{ font-family: 'Times New Roman', Times, serif; font-size: 12pt; color: black; line-height: 1.2; }}
        h3 {{ text-transform: uppercase; font-weight: bold; margin-top: 15px; border-bottom: 1px solid black; }}
    </style></head>
    <body>
        {relatorio.replace(chr(10), '<br>').replace('### ', '<h3>').replace('📍 ', '<h3>')}
    </body>
    </html>
    """
    st.download_button("📥 Baixar Vistoria para Word", html_format, "vistoria_tecnica.doc", "application/msword")
